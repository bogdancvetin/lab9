const matrix: ReadonlyArray<ReadonlyArray<number>> = [
  ...
  function printMatrix(matrix: ReadonlyArray<ReadonlyArray<number>>): void {
    console.log("Матрица:");
    matrix.forEach(row => console.log(row.join(" ")));
  }

  printMatrix(matrix);
    function getUserInput(prompt: string, callback: (input: string) => void): void {
  ...
  function handleJaggedArrayInput(): void {
    const rows = [2, 3, 4];
    const jaggedArray: number[][] = [];
  
    function inputRow(rowIndex: number): void {
